...
// valores temporales de controle de flujo
int tem_sel[] = {0, 0};
bool salir_com = false; // siempre comienza con falso
...
void ler_serial() {
  ...
  if (Serial1.available() != 0) {
    ...
    /// Verificar si hay cambios en el mensaje de entrada
    while(salir_com == true){
      if(tem_sel[0] == var_ctrl[0] && tem_sel[1] == var_ctrl[1]){
        // leio a porta e continuo
    ...
    switch (var_ctrl[0]) {
      case 1: //Mover carro
        ...
        tem_sel[0] = var_ctrl[0];
        tem_sel[1] = var_ctrl[1];
        salir_com = true;
        break;
      case 2: // Mover camara
        Serial.println(data_send);
